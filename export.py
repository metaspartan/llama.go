...
def load_hf_model(model_path):
    ...
    config.n_kv_heads = hf_model.config.num_key_value_heads  # Use the correct number of KV heads
    ...
    def permute_reverse(w, n_heads=config.n_heads, n_kv_heads=config.n_kv_heads, dim1=config.dim, dim2=config.dim, is_kv=False):
        if is_kv:
            # For key and value projections
            head_dim = 64
            n_heads_to_use = n_kv_heads
            # Reshape to proper dimensions for KV heads
            return w.reshape(dim1, n_heads_to_use * head_dim)
        else:
            # For query projections
            head_dim = 64
            n_heads_to_use = n_heads
            # Reshape to proper dimensions for Q heads
            return w.reshape(dim1, n_heads_to_use * head_dim)
    ...
    for layer in model.layers:
        ...
        layer.attention.wq.weight = nn.Parameter(
            permute_reverse(hf_dict[f"model.layers.{i}.self_attn.q_proj.weight"], is_kv=False)
        )
        layer.attention.wk.weight = nn.Parameter(
            permute_reverse(hf_dict[f"model.layers.{i}.self_attn.k_proj.weight"], is_kv=True)
        )
        layer.attention.wv.weight = nn.Parameter(
            permute_reverse(hf_dict[f"model.layers.{i}.self_attn.v_proj.weight"], is_kv=True)
        )
        layer.attention.wo.weight = nn.Parameter(
